...
def gep_objective_function(individual, import_data):  # evalueate each individual
    ...
    pos_str = reshape_variables.reshape_variables_moabc(individual)
    gen_decision = np.array(pos_str["generation"])
    invest_decision = np.array(pos_str["investment"])
    ...
    sum_viol_peak_demand = np.sum(viol_peak_demand)

    if sum_viol_peak_demand != 0:
    ...
    cumsum_constraints = np.sum(constraints)

    # trial = 0
    # if cumsum_constraints > 0:
    #     trial = 999

    if total_cost < 0 or emission < 0:
        raise Exception(f"NEGATIF COST? | COST: ({total_cost} ve EM: {emission})")

    return y["cost"], cumsum_constraints  # + cumsum_constraints
